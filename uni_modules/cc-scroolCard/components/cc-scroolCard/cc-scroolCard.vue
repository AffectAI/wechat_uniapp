<template>
	...
	<swiper style="padding-top: 55px;" class="poster_swiper" previous-margin="110rpx" :current="swiperIndex" next-margin="110rpx"
		@change="onSwiperChange">
		<swiper-item v-for="(item, index) of dataInfo.headImgs" :key="index">
			<view class="goods_info_box" :class="{ active: swiperIndex == index }" @click="next(item)" style="position: relative;">
				<image class="goods_image" :src="item.url" mode="aspectFit"></image>
				<view style="text-align: center;font-size: 22px;color: #fff">{{item.name}}</view>
			...
			<view style="position: absolute;bottom: -80px;left: 0;right: 0;margin-left: auto;margin-right: auto;z-index: 600;height: 95px;">
				<view class="triangle"></view>
				<button 
				style="background-color: rgb(101,246,180);margin-top: 50px;font-size:14px;">{{item.name}}</button>
			</view>
		</swiper-item>
	</swiper>
