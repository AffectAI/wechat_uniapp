<template>
	<view class="page26-wrap" >
		<view class="models">
			<view style="background-color: #ffffff69;width: 100%;margin: 2px auto 20px auto;padding: 7px;border-radius: 5px;"> {{liuyanTxt}} </view>
		...
		<view @click="endSound" 
			v-if="timeCount >= 20"
			style="width: 80px;text-align: center;line-height: 45px;line-height: 45px;font-size: 17px;border-radius: 5px;background-color: #58a5f3;position: absolute;right: 0;left: 0;margin: auto;bottom: 30%;">
		完成</view>
		...
		methods: {
			...
			initSound() {
				...
				this.ctx.onStop( (res) => {
				  ...
				  this.$emit('setRecore', {recoreTempPath: res.tempFilePath, audioType: 'audio', start_time: startTime, times: '', postion: 'nohapply_share'});
				  this.ctx.stop();
